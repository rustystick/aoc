...
function solution(input) {
    ...
    while (queue.size()) {
        ...
        if (r === grid.length - 1 && c === grid[0].length - 1 && stepSoFar >= 4) {
            return heat;
        }
        ...
        if (stepSoFar < 4 && !(dr === 0 && dc === 0)) {
            ...
            for (const [ndr, ndc] of [[0, 1], [0, -1], [1, 0], [-1, 0]]) {
                if (!(ndr === -dr && ndc === -dc)) {
                    // could continue until 10
                    if (ndr === dr && ndc === dc) {
                        if (stepSoFar < 10) {
                            let [nnr, nnc] = [r + ndr, c + ndc];
                            if (inBound([nnr, nnc])) {
                                queue.enqueue([grid[nnr][nnc] + heat, nnr, nnc, ndr, ndc, stepSoFar + 1]);
                            }
